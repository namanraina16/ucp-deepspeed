...
class UniversalCheckpointingAnalysis(TensorBoardAnalysis):
    ...
    def set_names(self, path_name):
        ...
        # Now extracts 5 groups: TP, PP, DP, SP, and ZeRO Stage
        tp, pp, dp, sp, z = match.groups()

        # Update the label to show the ZeRO stage on the image
        self._label_name = f"TP:{tp}, PP:{pp}, DP:{dp}, SP:{sp}, ZeRO:{z}"
        self._csv_name = f"uc_out_tp{tp}_pp{pp}_dp{dp}_sp{sp}_z{z}"
    ...
    def path_regex(self):
        # Updated regex: looks for 'z' followed by digits at the end
        return '.*tp(\d+).*pp(\d+).*dp(\d+).*sp(\d+).*z(\d+)'