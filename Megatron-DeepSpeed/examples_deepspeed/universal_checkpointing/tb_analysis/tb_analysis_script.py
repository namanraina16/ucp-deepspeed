...
def main():
    ...
    # Prefer source_ over target_ when both exist for the same (tp,pp,dp,sp,z)
    rx = re.compile(r'.*tp(\d+).*pp(\d+).*dp(\d+).*sp(\d+).*z(\d+)')
    ...
    for p in tb_log_paths:
        ...
        if k not in by_key:
            ...
            already = by_key[k]
            if ("/source_" in p) and ("/source_" not in already):
                by_key[k] = p

    tb_log_paths = list(by_key.values())
